...
export function installNginxCertbot(): void {
  run('apt-get install -y nginx certbot python3-certbot-nginx');
  run('systemctl enable nginx');
}

/** Writes the Nginx virtual host config for the panel and reloads Nginx. */
export function configureNginxPanel(config: InstallConfig): void {
  const { panelDomain, installPath } = config;

  if (existsSync('/etc/nginx/sites-enabled/default')) {
    unlinkSync('/etc/nginx/sites-enabled/default');
  }
