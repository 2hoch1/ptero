...
import { GITHUB_REPO, refreshVersionCache } from '@/lib/version-check.js';
...
export const updateCommand = defineCommand({
  ...
  async run() {
    p.intro(pc.bgCyan(pc.black('  ptero update  ')));
    ...
    try {
      ...
      spinner.stop(pc.red('Failed to reach GitHub'));
      throw err;
    }
    ...
    spinner.stop(`New version found: ${pc.green(`v${latest}`)} (current: v${currentVersion})`);
    ...
    try {
      ...
      downloadSpinner.stop(pc.red('Download failed'));
      throw err;
    }
    ...
    p.outro(pc.green(`Updated to v${latest}. Run 'ptero --version' to confirm.`));
  },
});
